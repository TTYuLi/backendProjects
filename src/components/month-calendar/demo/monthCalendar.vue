...
<script>
import calendar from './monthCalendar.js'
export default {
    props: {
        // 多选模式
        multi: {
    ...
    methods: {
        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                ...
                //判断当前年月 是否 与 选中的 年月一致！如果不是、当前年月的日期 去掉
                // console.log("this.multiDays",this.multiDays);
                this.multiDays = this.multiDays.filter(v=>{
                    return this.year === v[0] && this.month === v[1]-1
                })
...
.monthCalendar td.week{
    font-size:10px;
    pointer-events:none !important;
    cursor: default !important;    
}
.monthCalendar td.disabled {
    color: #ccc;
    pointer-events:none !important;
    cursor: default !important;
}
.monthCalendar td.disabled div{
    color: #ccc;
}
.monthCalendar td span{
    display:block;
    max-width:40px;
    height:26px;
    font-size: 16px;
...
.monthCalendar td:not(.selected) span:not(.red):hover{
    background:#f3f8fa;
    color:#444;
}
.monthCalendar td:not(.selected) span.red:hover{
    background:#f9efef;
}

.monthCalendar td:not(.disabled) span.red{
    color:#ea6151;
}
.monthCalendar td.selected span{
    background-color: #5e7a88;
    color: #fff;
}
.monthCalendar td .text{
    position: absolute;
    top:28px;
    left:0;
    right:0;
...
.monthCalendar td .isGregorianFestival,
.monthCalendar td .isLunarFestival{
    color:#ea6151;
}
.monthCalendar td.selected span.red{
    background-color: #ea6151;
    color: #fff;
}
.monthCalendar td.selected span.red:hover{
    background-color: #ea6151;
    color: #fff;
}
.monthCalendar thead td {
